...
/** forward taint to find dispatch
    ...
    while(!ops.empty()) {
        ...
        if(!dump.empty() && curIns == dump.back()) {
            dump.pop_back();
            ...
            for(uint64_t i = 1; i < size; i++) {
                if(labels[i]) {
                    ...
                    if(labelSize(taintState, label) == 1) {
                        continue;
                    }
                    ...
                    uint64_t allSmaller = mergeLabels(taintState, min - 1, label);
                    //fprintf(stderr, "%llx vs %llx\n",(unsigned long long) allSmaller,(unsigned long long) maxLabel);
                    if(allSmaller == maxLabel) {
                        labels[i] = 0;
                        labels[maxLabel] = 1;
                    }
            ...
            uint8_t *labels1 = getLabelArray(taintState);
            for(uint64_t i = 1; i < size; i++) {
                labels1[i] = labels[i];
            }
            ...
            outputCondensedLabels(taintState, labels1);
            delete[] labels1;
