    ...
    while(!ops.empty()) {
        ...
        if(!dispatches.empty() && curIns == dispatches.back().first) {
            ...
            if(dispatches.empty() || curDispatch.first != dispatches.back().first) {
                ...
                if(dispatchLabel == 0) {
                    dispatchLabel = getNewLabel(backTaint);
                    vip[dispatchLabel] = curDispatch.second;

                    if(baseReg != LYNX_INVALID) {
                        taintReg(backTaint, baseReg, tid, dispatchLabel);
                    }

                    if(indexReg != LYNX_INVALID) {
                        taintReg(backTaint, indexReg, tid, dispatchLabel);
                    }

                    if(segReg != LYNX_INVALID) {
                        taintReg(backTaint, segReg, tid, dispatchLabel);
                    }
                    printf(" < new (%llx) %llu %llx %s\n", (unsigned long long) dispatchLabel, (unsigned long long) curIns, (unsigned long long) addr, info->mnemonic);
                }
                else if(labelSize(backTaint, dispatchLabel) == 1) {
                    controlFlow[make_pair(curDispatch.second, vip[dispatchLabel])]++;
                    vip[dispatchLabel] = curDispatch.second;
                    printf(" < re-use (%llx) %llu %llx %s\n", (unsigned long long) dispatchLabel, (unsigned long long) curIns, (unsigned long long) addr, info->mnemonic);
                }
                else {
                    printBaseLabels(backTaint, dispatchLabel);
                    uint64_t connections = dispatchLabel;                   
                    dispatchLabel = getNewLabel(backTaint);
                    vip[dispatchLabel] = curDispatch.second;
 
                    while(connections != 0) {
                        controlFlow[make_pair(curDispatch.second, vip[popLabel(backTaint, &connections)])]++;
                    }
