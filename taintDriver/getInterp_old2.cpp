...
int main(int argc, char *argv[]) {
    ...
    TaintState *vipTaint = initTaint(readerState);
        ...
        if(propagate) {
                ...
                if(taint != 0) {
                    ...
                    if(found) {
                        ...
                        if(op->mem.base != LYNX_INVALID && op->mem.base != LYNX_RIP && op->mem.base != LYNX_RSP) {
                            ...
                            vip = getCombinedRegTaint(vipTaint, baseReg, curEvent.ins.tid, vip);
                        ...
                        if(op->mem.seg != LYNX_INVALID) {
                            ...
                            vip = getCombinedRegTaint(vipTaint, segReg, curEvent.ins.tid, vip);
                        ...
                        if(op->mem.index != LYNX_INVALID && op->mem.index != LYNX_RIP && op->mem.index != LYNX_RSP) {
                            ...
                            vip = getCombinedRegTaint(vipTaint, indexReg, curEvent.ins.tid, vip);
                        ...
                        taintOperandList(vipTaint, info.srcOps, info.srcOpCnt, curEvent.ins.tid, 0);
                        taintOperandList(vipTaint, info.readWriteOps, info.readWriteOpCnt, curEvent.ins.tid, 0);
                        taintAddrCalcList(vipTaint, info.dstOps, info.dstOpCnt, curEvent.ins.tid, 0);

                        if(vip == 0 || labelSize(vipTaint, vip) != 1) {
                            uint64_t confirmed = getLabelIntersect(vipTaint, vip, candidateVips);
                            if(labelSize(vipTaint, confirmed) == 1) {
                                vip = confirmed;
                            }
                            else {
                                if(confirmed == 0) {
                                    vip = getNewLabel(vipTaint);
                                    printf("Creating %llx at %llu\n", (unsigned long long) vip, (unsigned long long) curIns); 
                                }
                                else {  
                                    vip = getNewLabel(vipTaint);
                                    printf("Combining ");
                                    while(confirmed != 0) {
                                        printf("%llx ", (unsigned long long) popLabel(vipTaint, &confirmed));
                                    }
                                    printf("into %llx", (unsigned long long) vip);
                                }
                            }
                        }
                        if(baseReg != LYNX_INVALID) {
                            taintReg(vipTaint, baseReg, curEvent.ins.tid, vip);
                        }
                        if(indexReg != LYNX_INVALID) {
                            taintReg(vipTaint, indexReg, curEvent.ins.tid, vip);
                        }
                        if(segReg != LYNX_INVALID) {
                            taintReg(vipTaint, segReg, curEvent.ins.tid, vip);
                        }                            
                        //taintMem(vipTaint, op->mem.addr, op->mem.size, vip);
                    }
                }
            }

            uint64_t taint = propagateForward(bytecodeTaint, &curEvent, &info);
            uint64_t vipLabel = propagateForward(vipTaint, &curEvent, &info);
            /*if(vipLabel) {
            ...
            if(taint && (info.insClass == XED_ICLASS_JMP || info.insClass == XED_ICLASS_JMP_FAR || info.insClass == XED_ICLASS_CALL_FAR || info.insClass == XED_ICLASS_CALL_NEAR) && (dispatch.empty() || dispatch.find(curEvent.ins.addr) != dispatch.end())) {
                ...
                if(indirectJmp && taint != genericLabel && labelSize(bytecodeTaint, taint) == 1) {
                    uint64_t vip = getLabelIntersect(vipTaint, vipLabel, candidateVips);
                    if(vip == 0) {
                        vip = vipLabel;
                        candidateVips = mergeLabels(vipTaint, vipLabel, candidateVips);
                    }
                    printf(" > [%llx](%llx:%llu) %s %s %llu %llx %s\n", (unsigned long long) vip, (unsigned long long) taint, (unsigned long long) labelMap[taint], fetchStrFromId(readerState, curEvent.ins.srcId), fetchStrFromId(readerState, curEvent.ins.fnId), (unsigned long long) curIns, (unsigned long long) curEvent.ins.addr, info.mnemonic);
