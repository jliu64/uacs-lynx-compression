...
int main(int argc, char *argv[]) {
    ...
    while(nextEvent(readerState, &curEvent)) {
        ...
        if(propagate) {
            ...
            if(taint && (curInfo->insClass == XED_ICLASS_JMP || curInfo->insClass == XED_ICLASS_JMP_FAR || curInfo->insClass == XED_ICLASS_CALL_FAR || curInfo->insClass == XED_ICLASS_CALL_NEAR) && (dispatch.empty() || dispatch.find(curEvent.ins.addr) != dispatch.end())) {
                ...
                //if(indirectJmp) {// && labelIsSubsetOf(forwardTaint, taint, world)) {
                if(indirectJmp && taint != genericLabel && labelSize(forwardTaint, taint) == 1) {
                    printf(" > (%llx:%llu) %s %s %llu %llx %s\n", (unsigned long long) taint, (unsigned long long) labelMap[taint], fetchStrFromId(readerState, curEvent.ins.srcId), fetchStrFromId(readerState, curEvent.ins.fnId), (unsigned long long) curIns, (unsigned long long) curEvent.ins.addr, curInfo->mnemonic);
    ...
    set<uint64_t> subgraphs;
    set<uint64_t> graphs;
    unordered_map<uint64_t, uint64_t> parents;
    ...
    while(!ops.empty()) {
        ...
        /*set<uint64_t> srcGraphs;
        getSrcTaint(backTaint, info, tid, srcGraphs);*/
        ...
        if(!jmps.empty() && curIns == jmps.back().first) {
            ...
            newJmps = mergeLabels(backTaint, newJmps, jmpLabel);
            ...
            subgraphs.insert(jmpLabel);
            graphs.insert(jmpLabel);
            parents[jmpLabel] = jmpLabel;
            jmps.pop_back();
        }
        else {
            if(taint == 0 || graphs.empty() || subgraphs.find(taint) != subgraphs.end()) {
                //printf("empty\n");
                continue;
            }
            ...
            if(total == 0 || !labelIsSubsetOf(backTaint, taint, total)) {
                /*printf("looking for: ");
                printBaseLabels(backTaint, taint);
                printf("found: ");
                printBaseLabels(backTaint, total);
                assert(0);*/
            ...
            /*uint64_t noControlFlow = getLabelIntersect(backTaint, taint, newJmps);
            if(noControlFlow != 0 && labelSize(backTaint, taint) > 1) {
                newJmps = subtractLabel(backTaint, noControlFlow, newJmps);
                uint64_t seenInstructions = subtractLabel(backTaint, noControlFlow, taint);
                uint64_t nextInstruction;
                if(seenInstructions != 0) {
                    printf("using last seen\n");
                    nextInstruction = popLabel(backTaint, &seenInstructions);
                }
                else {
                    printf("using first non-seen\n");
                    nextInstruction = getFirstLabel(backTaint, noControlFlow);
                    noControlFlow = subtractLabel(backTaint, nextInstruction, noControlFlow);
                }
                //if multiple join at the same instruction, connect them to that instruction
                while(noControlFlow != 0) {
                    uint64_t from = jmpMap[popLabel(backTaint, &noControlFlow)];
                    uint64_t to = jmpMap[nextInstruction];
                    printf("Connecting %llx to %llx at %llu\n", (unsigned long long) from, (unsigned long long) to, (unsigned long long) curIns);
                    controlFlow[make_pair(from, to)]++;
                }
            }*/
            //check if graphs have merged
        }
    }
