#include <cstdio>
#include <vector>
#include <utility>
#include <set>
#include <cassert>
#include <unordered_map>
#include <cstring>
#include <string>

//track read size and create a new label when read size doesn't match
//every tainted write, write a new label

//taint write with generic label, taint read with  regular label
// watch size of read?

// watch for VIP?

using namespace std;

extern "C" {
    #include <Reader.h>
    #include <Taint.h>
    #include <cfg.h>
}

char *trace = NULL;
set<uint64_t> dispatch;
vector< pair<uint64_t, uint32_t> > bytecode;
vector<char *> sources;
bool coalesce = false;

struct pairHash {
    //std::size_t operator() (const std::pair<uint64_t, uint64_t> &p) const {
    inline std::size_t operator()(const std::pair<uint64_t, uint64_t> & p) const {
        return p.first * p.second;
    }   
};

void printUsage(char *program) {
    printf("Usage: %s -t <trace> [-bd]\n", program);
    printf("  d <addr1 addr2 ...> - List of dispatch addresses\n");
    printf("  b <base1[size1] base2[size2] ...> - List identifying bytecode locations in memory\n");
    printf("* s <source1 source2 ...> - source files of interest executed by the interpereter\n");
    printf("  h - show this message\n");
    printf("  c - coalesce bytecode instructions into basic blocks\n");
    printf("* is required\n");
}

void parseCommandLine(int argc, char *argv[]) {
    int i = 1;
    while(i < argc) {
        if(argv[i][0] == '-' && strlen(argv[i]) == 2) {
            switch(argv[i][1]) {
                case 'b':
                    i++;
                    for(; i < argc && argv[i][0] != '-'; i++) {
                        char *nextPtr = NULL;
                        uint64_t base = strtoull(argv[i], &nextPtr, 16);
                        if(*nextPtr != '[' || base == 0) {
                            fprintf(stderr, "Invalid Format %s, should be base_addr[size] (base is in hex, size is not)\n", argv[i]);
                            continue;
                        }   
                        nextPtr++;
                        uint32_t size = strtoul(nextPtr, &nextPtr, 10);
                        if(*nextPtr != ']' || size == 0) {
                            fprintf(stderr, "Invalid Format %s, should be base_addr[size] (base is in hex, size is in decimal)\n", argv[i]);
                            continue;
                        }
                        bytecode.push_back(make_pair(base, size));
                    }
                    break;
                case 'd':
                    i++;
                    for(; i < argc && argv[i][0] != '-'; i++) {
                        uint64_t addr = strtoull(argv[i], NULL, 16);
                        printf("looking for %llx\n", (unsigned long long) addr);
                        if(addr != 0) {
                            dispatch.insert(addr);
                        }
                    }
                    break;
                case 't':
                    i++;
                    trace = argv[i];
                    i++;
                    break;
                case 's':
                    i++;
                    for(; i < argc && argv[i][0] != '-'; i++) {
                        sources.push_back(argv[i]);
                    }
                    break;
                case 'h':
                    printUsage(argv[0]);
                    exit(0);
                case 'c':
                    coalesce = true;
                    i++;
                    break;
                default:
                    printf("Unknown Command Line Argument %s\n", argv[i]);
                    i++;
                    break;
            }
        }
        else {
            printf("Unknown Command Line Argument %s\n", argv[i]);
            i++;
        }
    }

    return;
}

char *fun_name(Function *ifun) {
    const char *prefix = "f_";
    char buf[20];


    assert(ifun != NULL);
  
    if (ifun->name != NULL) {
        return strdup(ifun->name);
    }   
  
    strcpy(buf, prefix);
    snprintf(buf+strlen(prefix), 16, "%d", ifun->id);
  
    return strdup(buf);
}

char * printIns(cfgInstruction *iins, const char *ins_str, ReaderState *rState) {
    char this_ins_str[128];
    char *cat_ins_str;
    int cat_ins_len;

    assert(iins && ins_str);

    InsInfo iinfo;
    initInsInfo(&iinfo);
    fetchInsInfo(rState, &(iins->event.ins), &iinfo);

    snprintf(this_ins_str, 128, "%llx: %s %d %d", (unsigned long long)iins->event.ins.addr, iinfo.mnemonic, iins->block->fun->id, iins->event.ins.tid);

    cat_ins_len = strlen(ins_str) + strlen(this_ins_str) + 3;  /* 3 = strlen("\\n") + NUL */
    cat_ins_str = new char[cat_ins_len];
    strcpy(cat_ins_str, ins_str);
    strcat(cat_ins_str, "\\n");
    strcat(cat_ins_str, this_ins_str);
    return cat_ins_str;

}

void printEdge(FILE *fp, uint64_t taint, Edge *curEdge, ReaderState *rState) {
    const char *edgstyle;
    const char *edgcolor = "black";

    assert(curEdge != NULL);

    if (curEdge->etype == ET_ENTRY || curEdge->etype == ET_EXIT || curEdge->etype == ET_LINK || curEdge->etype == ET_DYNAMIC) {
        edgstyle = "dashed";  /* pseudo-edges */
    } else if (curEdge->etype == ET_EXCEPTION) {
        edgcolor = "red";
        edgstyle = "solid";
    } else {
        edgstyle = "solid";
    }

    fprintf(fp, "    B%lld%d -> B%lld%d [style=%s, color=\"%s\", label=\" ct:%ld\"];\n", 
        (unsigned long long) taint, curEdge->from->id, (unsigned long long) taint, curEdge->to->id, edgstyle, edgcolor, curEdge->count);

}

void printBlk(FILE *fp, uint64_t taint, Bbl *curBlk, ReaderState *rState) {
    const char *bblstyle;
    char buf[128] = {0};
    const char *label = buf;
    cfgInstruction *iins;

    assert(curBlk != NULL);


    if( curBlk->first != NULL ){
        label = strdup(fetchStrFromId(rState, curBlk->first->event.ins.fnId));
    } else {
        label = "";
    }

    for (iins = curBlk->first; iins != NULL; iins = iins->next) {
        label = printIns(iins, label, rState);
    }

    if (curBlk->btype == BT_ENTRY || curBlk->btype == BT_EXIT) {  /* pseudo-blocks */
        bblstyle = "dashed";
    }
    else {
        bblstyle = "solid";
    }

    fprintf(fp, "    B%lld%d [shape=box, style=%s, label=\"%s\"];\n", (unsigned long long) taint, curBlk->id, bblstyle, label);

    for (Edge *curEdge = curBlk->succs; curEdge != NULL; curEdge = curEdge->next) {
        printEdge(fp, taint, curEdge, rState);
    }
}

void printFun(FILE *fp, uint64_t taint, Function *curFun, ReaderState *rState) {
    char *fname;

    assert(curFun != NULL);

    // Get function name
    fname = fun_name(curFun);
    fprintf(fp, "//Starting Function %s", fname);

    /*fprintf(fp, "\n");
    fprintf(fp, "  subgraph %d {\n", curFun->id);
    fprintf(fp, "    label = \"%d\";\n", curFun->id);*/

    /* print all the blocks */
    for (Bbl *curBlk = curFun->first; curBlk != NULL; curBlk = curBlk->next) {
        printBlk(fp, taint, curBlk, rState);
    }

    fprintf(fp, "//Ending Function %s\n", fname);
    //fprintf(fp, "  }\n");
}

void outputDot(FILE *dotfile, uint64_t taint, cfgState *cfgStructArg, ReaderState *rState) {
    cfg *cfgroot = getCFG(cfgStructArg);

    assert(cfgroot != NULL);

    fprintf(dotfile, "subgraph \"cluster_N%llx\" { \n", (unsigned long long) taint);
    fprintf(dotfile, "graph[style=dotted];\n");
    fprintf(dotfile, "label=\"N%llx\";\n", (unsigned long long) taint);

    for (Function *curFun = cfgroot->startingpoint; curFun != NULL; curFun = curFun->next) {
        printFun(dotfile, taint, curFun, rState);
    }

    fprintf(dotfile, "}\n");
}

void printRegOp(FILE *interpTrace, ReaderState *readerState, const char *prefix, LynxReg reg, uint32_t tid) {
    reg = LynxReg2FullLynxIA32EReg(reg);
    fprintf(interpTrace, "%s:%s=", prefix, LynxReg2Str(reg));

    const uint8_t *val = getRegisterVal(readerState, reg, tid);

    int i;
    for(i = LynxRegSize(reg) - 1; i >= 0; i--) {
        fprintf(interpTrace, "%02x", val[i]);
    }   

    fprintf(interpTrace, " ");
}

void printMemOp(FILE *interpTrace, ReaderState *readerState, const char *prefix, ReaderOp *op, uint32_t tid) {
    if(!op->mem.addrGen) {
        fprintf(interpTrace, "%s[%llx]=", prefix, (unsigned long long) op->mem.addr);
        uint8_t *buf = new uint8_t[op->mem.size];

        getMemoryVal(readerState, op->mem.addr, op->mem.size, buf);

        int i;
        for(i = op->mem.size - 1; i >= 0; i--) {
            fprintf(interpTrace, "%02x", buf[i]);
        }   
        fprintf(interpTrace, " ");

        free(buf);
    }   

    if(op->mem.seg != LYNX_INVALID) {
        printRegOp(interpTrace, readerState, "R", (LynxReg) op->mem.seg, tid);
    }   
    if(op->mem.base != LYNX_INVALID) {
        printRegOp(interpTrace, readerState, "R", (LynxReg) op->mem.base, tid);
    }   
    if(op->mem.index != LYNX_INVALID) {
        printRegOp(interpTrace, readerState, "R", (LynxReg) op->mem.index, tid);
    }   
}

/*void getSrcTaint(TaintState *state, InsInfo *info, uint32_t tid, set<uint64_t> &taint) {
    ReaderOp *op = info->srcOps;
    for(int i = 0; i < info->srcOpCnt; i++) {
        if(op->type == REG_OP) {
            taint.insert(getCombinedRegTaint(state, (LynxReg) op->reg, tid, 0));
        }
        else if(op->type == MEM_OP) {
            taint.insert(getMemTaintLabel(state, op->mem.addr, op->mem.size, 0);

            if(op->mem.base != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.base, tid, 0));
            }
            if(op->mem.index != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.index, tid, 0));
            }
            if(op->mem.seg != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.seg, tid, 0));
            }
        }
    }

    ReaderOp *op = info->readWriteOps;
    for(int i = 0; i < info->readWriteCnt; i++) {
        if(op->type == REG_OP) {
            taint.insert(getCombinedRegTaint(state, (LynxReg) op->reg, tid, 0));
        }
        else if(op->type == MEM_OP) {
            taint.insert(getMemTaintLabel(state, op->mem.addr, op->mem.size, 0);

            if(op->mem.base != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.base, tid, 0));
            }
            if(op->mem.index != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.index, tid, 0));
            }
            if(op->mem.seg != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.seg, tid, 0));
            }
        }
    }

    ReaderOp *op = info->dstOps;
    for(int i = 0; i < dstOpCnt; i++) {
        else if(op->type == MEM_OP) {
            if(op->mem.base != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.base, tid, 0));
            }
            if(op->mem.index != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.index, tid, 0));
            }
            if(op->mem.seg != LYNX_INVALID) {
                taint.insert(getCombinedRegTaint(state, (LynxReg) op->mem.seg, tid, 0));
            }
        }
    }
}*/

int main(int argc, char *argv[]) {
    parseCommandLine(argc, argv);

    //check for current implementation
    if(trace == NULL || sources.empty()) {
        printUsage(argv[0]);
        exit(1);
    }

    ReaderState *readerState = initReader(trace, 0);
    TaintState *forwardTaint = initTaint(readerState); 
    TaintState *dispatchTaint = initTaint(readerState);
    TaintState *backTaint = initTaint(readerState);

    ReaderEvent curEvent;
    InsInfo info;
    InsInfo *curInfo = &info;
    initInsInfo(curInfo);

    set<uint64_t> activeDescriptors;

    string openedFile;
    uint32_t syscallTid = 0;
    uint8_t checkOpenResult = 0;
    uint64_t curIns = 0;
    uint8_t propagate = 0;

    uint64_t lastTaintedIns = 0;
    uint64_t lastTaintedAddr = 0;
    uint64_t lastTaintedSize = 0;
    uint64_t lastTaintLabel = 0;
    uint64_t genericLabel = getNewLabel(forwardTaint);

    set<uint64_t> taintSet;
    //unordered_map<uint32_t, uint64_t> prevLabel;
    vector< pair<uint64_t, uint64_t> > jmps;
    vector< pair<pair<uint64_t, pair<uint64_t, uint32_t> >, InsInfo *> > ops;
    vector< pair<uint64_t, uint64_t> > dispatches;
    unordered_map<uint64_t, uint64_t> labelMap;
    unordered_map<uint32_t, vector< pair<uint64_t, uint64_t> > > labelSeq;
    set<uint32_t> tids;
    uint64_t world = genericLabel;
    unordered_map<uint64_t, uint32_t> labelSizes;

    unordered_map<uint64_t, uint64_t> identities;

    unordered_map<uint32_t, uint64_t> lastDispatch;

    unordered_map<uint64_t, cfgState *> cfgs;
    cfgState *curCfg = NULL;

    int nameSize = strlen(trace);
    char *interpTraceFilename = new char[nameSize + 5]();
    char *cfgFilename = new char[nameSize + 14]();
    strncpy(interpTraceFilename, trace, nameSize);
    strncpy(interpTraceFilename + nameSize, ".interp.ascii", 13);
    FILE *interpTrace = fopen(interpTraceFilename, "w");
    strncpy(cfgFilename, trace, nameSize);
    strncpy(cfgFilename + nameSize, ".dot", 4);
    FILE *cfg = fopen(cfgFilename, "w");

    while(nextEvent(readerState, &curEvent)) {
        curIns++;
        if(curEvent.type ==  EXCEPTION_EVENT) {
            continue;
        }
        assert(curEvent.type == INS_EVENT);

        if(curCfg != NULL) {
            ReaderOp *curOp = curInfo->readWriteOps;
            for(int i = 0; i < curInfo->readWriteOpCnt; i++) {
                if(curOp->type == MEM_OP) {
                    printMemOp(interpTrace, readerState, "MW", curOp, curEvent.ins.tid);
                }
                else if(curOp->type == REG_OP) {
                    printRegOp(interpTrace, readerState, "W", curOp->reg, curEvent.ins.tid);
                }
                curOp = curOp->next;
            }

            curOp = curInfo->dstOps;
            for(int i = 0; i < curInfo->dstOpCnt; i++) {
                if(curOp->type == MEM_OP) {
                    printMemOp(interpTrace, readerState, "MW", curOp, curEvent.ins.tid);
                }
                else if(curOp->type == REG_OP) {
                    printRegOp(interpTrace, readerState, "W", curOp->reg, curEvent.ins.tid);
                }
                curOp = curOp->next;
            }
            fprintf(interpTrace, "\n");
        }

        if(propagate) {
            curInfo = new InsInfo;
            initInsInfo(curInfo);
            ops.push_back(make_pair(make_pair(curIns, make_pair(curEvent.ins.addr, curEvent.ins.tid)), curInfo));
        }

        fetchInsInfo(readerState, &curEvent.ins, curInfo);
        if(curCfg != NULL) {
            addInstructionToCFG(&curEvent, curCfg);

            fprintf(interpTrace, "%d ", curEvent.ins.tid);
            fprintf(interpTrace, "%llx ", (unsigned long long) curEvent.ins.addr);
            fprintf(interpTrace, "%s ", fetchStrFromId(readerState, curEvent.ins.srcId));
            fprintf(interpTrace, "%s ", fetchStrFromId(readerState, curEvent.ins.fnId));

            for(int i = 0; i < curEvent.ins.binSize; i++) {
                fprintf(interpTrace, "%02x ", curEvent.ins.binary[i]);
            }

            fprintf(interpTrace, "%s ", curInfo->mnemonic);

            ReaderOp *curOp = curInfo->srcOps;
            for(int i = 0; i < curInfo->srcOpCnt; i++) {
                if(curOp->type == MEM_OP) {
                    printMemOp(interpTrace, readerState, "MR", curOp, curEvent.ins.tid);
                }
                else if(curOp->type == REG_OP) {
                    printRegOp(interpTrace, readerState, "R", curOp->reg, curEvent.ins.tid);
                }
                curOp = curOp->next;
            }

            curOp = curInfo->readWriteOps;
            for(int i = 0; i < curInfo->readWriteOpCnt; i++) {
                if(curOp->type == MEM_OP) {
                    printMemOp(interpTrace, readerState, "MR", curOp, curEvent.ins.tid);
                }
                else if(curOp->type == REG_OP) {
                    printRegOp(interpTrace, readerState, "R", curOp->reg, curEvent.ins.tid);
                }
                curOp = curOp->next;
            }
        }

        if(checkOpenResult && curEvent.ins.tid == syscallTid) {
            checkOpenResult = 0;
            uint64_t fd = *((uint64_t *) getRegisterVal(readerState, LYNX_RAX, syscallTid));

            printf("' -- %llu\n", (unsigned long long) fd);

            int found = 0;
            int openedSize = openedFile.size();
            for(char *src : sources) {
                int srcSize = strlen(src);
                if(openedSize >= srcSize) {
                    printf("'%s' vs '%s'\n", openedFile.c_str() + openedSize - srcSize, src); 
                    if(strncmp(openedFile.data() + openedSize - srcSize, src, srcSize) == 0) {
                        found = 1;
                        break;
                    }
                }
            }
                
            if(found) {
                printf("found\n");
                activeDescriptors.insert(fd);
            }
            else {
                activeDescriptors.erase(fd);
            }
        }
        else if(curInfo->insClass == XED_ICLASS_SYSCALL) {
            uint64_t callNum = *((uint64_t *) getRegisterVal(readerState, LYNX_RAX, curEvent.ins.tid));
                
            //read
            if(callNum == 0) {
                uint64_t fd = *((uint64_t *) getRegisterVal(readerState, LYNX_RDI, curEvent.ins.tid));
                if(activeDescriptors.find(fd) != activeDescriptors.end()) {
                    uint64_t addr = *((uint64_t *) getRegisterVal(readerState, LYNX_RSI, curEvent.ins.tid));
                    uint64_t size = *((uint64_t *) getRegisterVal(readerState, LYNX_RDX, curEvent.ins.tid));
                    //uint64_t dataLabel = getNewLabel(forwardTaint);

                    taintMem(forwardTaint, addr, size, genericLabel);
                    if(!propagate) {
                        curInfo = new InsInfo;
                        initInsInfo(curInfo);
                        fetchInsInfo(readerState, &curEvent.ins, curInfo);
                        ops.push_back(make_pair(make_pair(curIns, make_pair(curEvent.ins.addr, curEvent.ins.tid)), curInfo));
                        propagate=1;
                    }
                    printf("reading %llu %llx[%llu]\n", (unsigned long long) fd, (unsigned long long) addr, (unsigned long long) size);
                }
            }
            //open
            else if(callNum == 2) {
                uint64_t filename = *((uint64_t *) getRegisterVal(readerState, LYNX_RDI, curEvent.ins.tid));
                openedFile.clear();

                printf("open '");

                char curChar;
                getMemoryVal(readerState, filename++, 1, (uint8_t *) &curChar);
                while(curChar != 0) {
                    printf("%c", curChar);
                    openedFile += curChar;
                    getMemoryVal(readerState, filename++, 1, (uint8_t *) &curChar);
                }

                syscallTid = curEvent.ins.tid;
                checkOpenResult = 1;
            }
        }

        if(propagate) {
            LynxReg segReg = LYNX_INVALID;
            uint64_t segTaint = 0;
            LynxReg baseReg = LYNX_INVALID;
            uint64_t baseTaint = 0;
            LynxReg indexReg = LYNX_INVALID;
            uint64_t indexTaint = 0;
            ReaderOp *reads[32];

            int num = getMemReads(curInfo, reads);
            if(num == 1) {
                ReaderOp *op = reads[0];
                if((op->mem.base != LYNX_INVALID && op->mem.base != LYNX_RIP && op->mem.base != LYNX_RSP) || (op->mem.index != LYNX_INVALID && op->mem.index != LYNX_RIP && op->mem.index != LYNX_RSP)) {
                    uint64_t taint = getMemTaintLabel(forwardTaint, op->mem.addr, op->mem.size, 0);
                    if(taint != 0) {
                        int found = bytecode.empty();
                        for(pair<uint64_t, uint32_t> &region : bytecode) {
                            uint64_t offset1 = op->mem.addr - region.first;
                            uint64_t offset2 = region.first - op->mem.addr;
                            if(offset1 < region.second || offset2 < op->mem.size) {
                                found = 1;
                                break;
                            }
                        }
                        if(found) {
                            if(labelSizes.find(taint) == labelSizes.end() || labelSizes[taint] != op->mem.size) {
                                uint64_t dataLabel = getNewLabel(forwardTaint);
                                labelMap[dataLabel] = curIns;
                                labelSizes[dataLabel] = op->mem.size;
                                printf("making new at %llu, old: %llx, new: %llx(%u)\n", (unsigned long long) curIns, (unsigned long long) taint, (unsigned long long) dataLabel, op->mem.size);
                                taintMem(forwardTaint, op->mem.addr, op->mem.size, dataLabel);
                            }
                            else {
                                printf("reuse at %llu: %llx(%u)\n", (unsigned long long) curIns, (unsigned long long) taint, labelSizes[taint]);
                            }

                            uint64_t identity = getNewLabel(dispatchTaint);
                            identities[identity] = curIns;
                            taintOperandList(dispatchTaint, curInfo->srcOps, curInfo->srcOpCnt, curEvent.ins.tid, 0);
                            taintOperandList(dispatchTaint, curInfo->readWriteOps, curInfo->readWriteOpCnt, curEvent.ins.tid, 0);
                            taintOperandList(dispatchTaint, curInfo->dstOps, curInfo->dstOpCnt, curEvent.ins.tid, 0);
                            
                            taintMem(dispatchTaint, op->mem.addr, op->mem.size, identity);

                            if(op->mem.base != LYNX_INVALID && op->mem.base != LYNX_RIP && op->mem.base != LYNX_RSP) {
                                baseReg = (LynxReg) op->mem.base;
                                baseTaint = getCombinedRegTaint(forwardTaint, (LynxReg)op->mem.base, curEvent.ins.tid, 0);

                                if(baseTaint != 0) {
                                    //addrTaint = baseTaint;
                                    taintReg(forwardTaint, baseReg, curEvent.ins.tid, 0);
                                }
                            }

                            if(op->mem.index != LYNX_INVALID && op->mem.index != LYNX_RIP && op->mem.index != LYNX_RSP) {
                                indexTaint = getCombinedRegTaint(forwardTaint, (LynxReg)op->mem.index, curEvent.ins.tid, 0);
                                indexReg = (LynxReg) op->mem.index;

                                if(indexTaint != 0) {
                                    //addrTaint = mergeLabels(forwardTaint, addrTaint, indexTaint);
                                    taintReg(forwardTaint, indexReg, curEvent.ins.tid, 0);
                                }
                            }

                            if(op->mem.seg != LYNX_INVALID) {
                                segTaint = getCombinedRegTaint(forwardTaint, (LynxReg)op->mem.seg, curEvent.ins.tid, 0);
                                segReg = (LynxReg) op->mem.seg;

                                if(segTaint != 0) {
                                    //addrTaint = mergeLabels(forwardTaint, addrTaint, segTaint);
                                    taintReg(forwardTaint, segReg, curEvent.ins.tid, 0);
                                }
                            }   
                        }
                    }
                }
            }

            uint64_t taint = propagateForward(forwardTaint, &curEvent, curInfo);
            uint64_t identTaint = propagateForward(dispatchTaint, &curEvent, curInfo);

            if(segTaint != 0 || indexTaint != 0 || baseTaint != 0) {
                ReaderOp *op = curInfo->dstOps;
                for(int i = 0; i < curInfo->dstOpCnt; i++) {
                    if(op->type == REG_OP) {
                        if(segReg == op->reg) {
                            segTaint = 0;
                        }
                        if(indexReg == op->reg) {
                            indexTaint = 0;
                        }
                        if(baseReg == op->reg) {
                            baseTaint = 0;
                        }
                    }
                    op = op->next;
                }
                if(segTaint != 0) {
                    addTaintToReg(forwardTaint, segReg, curEvent.ins.tid, segTaint);
                }
                if(indexTaint != 0) {
                    addTaintToReg(forwardTaint, indexReg, curEvent.ins.tid, indexTaint);
                }
                if(baseTaint != 0) {
                    addTaintToReg(forwardTaint, baseReg, curEvent.ins.tid, baseTaint);
                }
            }

            if(taint && (curInfo->insClass == XED_ICLASS_JMP || curInfo->insClass == XED_ICLASS_JMP_FAR || curInfo->insClass == XED_ICLASS_CALL_FAR || curInfo->insClass == XED_ICLASS_CALL_NEAR) && (dispatch.empty() || dispatch.find(curEvent.ins.addr) != dispatch.end())) {
                //only taint indirect jumps
                bool indirectJmp = false;
                ReaderOp *ops = curInfo->srcOps;
                int i;
                for(i = 0; i < curInfo->srcOpCnt; i++) {
                    if((ops->type == REG_OP && ops->reg != LYNX_RIP) || ops->type == MEM_OP) {
                        indirectJmp = true;
                        break;
                    }   
                    ops = ops->next;
                }   

                //make dispatch taint propagate same way forward taint does? I.e. don't zero everything when new?
                if(indirectJmp && taint != genericLabel && labelSize(forwardTaint, taint) == 1 && labelSize(dispatchTaint, identTaint) == 1 && lastDispatch[curEvent.ins.tid] <= identities[identTaint]) {
                    //assert(labelSize(dispatchTaint, identTaint) == 1);
                    printf(" > [%llu](%llx:%llu) %s %s %llu %llx %s\n", (unsigned long long) identities[identTaint], (unsigned long long) taint, (unsigned long long) labelMap[taint], fetchStrFromId(readerState, curEvent.ins.srcId), fetchStrFromId(readerState, curEvent.ins.fnId), (unsigned long long) curIns, (unsigned long long) curEvent.ins.addr, curInfo->mnemonic);
                    labelSeq[curEvent.ins.tid].push_back(make_pair(taint, curEvent.ins.addr));
                    tids.insert(curEvent.ins.tid);
                    jmps.push_back(make_pair(curIns, taint));
                    dispatches.push_back(make_pair(identities[identTaint], taint));
                    lastDispatch[curEvent.ins.tid] = identities[identTaint];

                    if(curCfg == NULL) {
                        curCfg = initCFG(readerState, getNumThreads(readerState));
                        addInstructionToCFG(&curEvent, curCfg);

                        fprintf(interpTrace, "%d ", curEvent.ins.tid);
                        fprintf(interpTrace, "%llx ", (unsigned long long) curEvent.ins.addr);
                        fprintf(interpTrace, "%s ", fetchStrFromId(readerState, curEvent.ins.srcId));
                        fprintf(interpTrace, "%s ", fetchStrFromId(readerState, curEvent.ins.fnId));

                        int i;
                        for(i = 0; i < curEvent.ins.binSize; i++) {
                            fprintf(interpTrace, "%02x ", curEvent.ins.binary[i]);
                        }
                        fprintf(interpTrace, "%s ", curInfo->mnemonic);
                        ReaderOp *curOp = curInfo->srcOps;
                        for(int i = 0; i < curInfo->srcOpCnt; i++) {
                            if(curOp->type == MEM_OP) {
                                printMemOp(interpTrace, readerState, "MR", curOp, curEvent.ins.tid);
                            }
                            else if(curOp->type == REG_OP) {
                                printRegOp(interpTrace, readerState, "R", curOp->reg, curEvent.ins.tid);
                            }
                            curOp = curOp->next;
                        }

                        curOp = curInfo->readWriteOps;
                        for(int i = 0; i < curInfo->readWriteOpCnt; i++) {
                            if(curOp->type == MEM_OP) {
                                printMemOp(interpTrace, readerState, "MR", curOp, curEvent.ins.tid);
                            }
                            else if(curOp->type == REG_OP) {
                                printRegOp(interpTrace, readerState, "R", curOp->reg, curEvent.ins.tid);
                            }
                            curOp = curOp->next;
                        }
                    }

                    //switch to merge
                    cfgs[taint] = curCfg;
                    finalizeCFG(curCfg);
                    curCfg = initCFG(readerState, getNumThreads(readerState));
                    //world = mergeLabels(forwardTaint, world, taint);
                    //controlFlow[make_pair(prevLabel[curEvent.ins.tid], taint)]++;
                    //prevLabel[curEvent.ins.tid] = taint;
                }   
            }
        }
    }

    unordered_map<uint64_t, uint64_t> vip;
    unordered_map<pair<uint64_t, uint64_t>, uint32_t, pairHash> controlFlow;
    //uint64_t active = 0;
    unordered_map<uint64_t, uint8_t> dispatchToSize;
    unordered_map<uint8_t, uint64_t> sizeToDispatch;
    unordered_map<uint8_t, uint64_t> activeBySize;
    unordered_map<uint64_t, uint64_t> vipRet;
    set<uint8_t> sizes;
    uint64_t notMergedWithVip = 0;
    while(!ops.empty()) {
        pair<pair<uint64_t, pair<uint64_t, uint32_t> >, InsInfo *> &opPair = ops.back();
        set<uint64_t> seenSubsets;
        uint64_t curIns = opPair.first.first;
        uint64_t addr = opPair.first.second.first;
        uint32_t tid = opPair.first.second.second;
        InsInfo *info = opPair.second;
        ops.pop_back();

        uint64_t taint = propagateBackward(backTaint, tid, info);
        /*taintReg(backTaint, LYNX_RSP, tid, 0);
        taintReg(backTaint, LYNX_RBP, tid, 0);*/
        if(!dispatches.empty() && curIns == dispatches.back().first) {
            pair<uint64_t, uint64_t> curDispatch = dispatches.back();
            dispatches.pop_back();

            if(dispatches.empty() || curDispatch.first != dispatches.back().first) {
                ReaderOp *op;
                getMemReads(info, &op);

                //printf(" < %llu %llx %s\n", (unsigned long long) curIns, (unsigned long long) addr, info->mnemonic);
                uint64_t dispatchLabel = 0; 

                LynxReg baseReg = LYNX_INVALID;
                if(op->mem.base != LYNX_INVALID && op->mem.base != LYNX_RIP && op->mem.base != LYNX_RSP) {
                    baseReg = (LynxReg) op->mem.base;
                    dispatchLabel = getCombinedRegTaint(backTaint, (LynxReg)op->mem.base, tid, dispatchLabel);
                }

                LynxReg indexReg = LYNX_INVALID;
                if(op->mem.index != LYNX_INVALID && op->mem.index != LYNX_RIP && op->mem.index != LYNX_RSP) {
                    indexReg = (LynxReg) op->mem.index;
                    dispatchLabel = getCombinedRegTaint(backTaint, (LynxReg)op->mem.index, tid, dispatchLabel);
                }

                LynxReg segReg = LYNX_INVALID;
                if(op->mem.seg != LYNX_INVALID) {
                    segReg = (LynxReg) op->mem.seg;
                    dispatchLabel = getCombinedRegTaint(backTaint, (LynxReg)op->mem.seg, tid, dispatchLabel);
                }

                dispatchLabel = getLabelIntersect(backTaint, activeBySize[op->mem.size], dispatchLabel);

                uint64_t curLabel = getNewLabel(backTaint);
                dispatchToSize[curLabel] = op->mem.size;
                vip[curLabel] = curDispatch.second;
                sizeToDispatch[op->mem.size] = mergeLabels(backTaint, activeBySize[op->mem.size], curLabel);
                activeBySize[op->mem.size] = mergeLabels(backTaint, activeBySize[op->mem.size], curLabel);
                sizes.insert(op->mem.size);
                //taint = mergeLabels(backTaint, taint, curLabel);
                taint = 0;
                vipRet[curLabel] = curLabel;

                printf("%llx: %llu %llx %s\n", (unsigned long long) curLabel, (unsigned long long) curIns, (unsigned long long) addr, info->mnemonic);

                uint64_t fromRet = curLabel;
                if(dispatchLabel != 0) {
                    activeBySize[op->mem.size] = subtractLabel(backTaint, dispatchLabel, activeBySize[op->mem.size]);
                    notMergedWithVip = subtractLabel(backTaint, dispatchLabel, notMergedWithVip);
                    printf(" < connecting %llx(%llx) to ", (unsigned long long) curLabel, (unsigned long long) curDispatch.second);
                    while(dispatchLabel != 0) {
                        uint64_t to = popLabel(backTaint, &dispatchLabel);
                        uint64_t toRet = vipRet[to];

                        if(fromRet != curLabel) {
                            controlFlow[make_pair(vip[fromRet], curDispatch.second)]++;
                        }

                        if(curDispatch.second == vip[to] && fromRet != curLabel && toRet != to && fromRet != toRet) {
                            continue;
                        }

                        printf("%llx(%llx) ", (unsigned long long) to, (unsigned long long) vip[to]);
                        controlFlow[make_pair(curDispatch.second, vip[to])]++;
                        fromRet = toRet;
                    }
                    vipRet[curLabel] = fromRet;
                    printf("at %llu\n", (unsigned long long) curIns);
                }
                else {
                    //
                    notMergedWithVip = mergeLabels(backTaint, notMergedWithVip, curLabel);
                }

                if(op->mem.base != LYNX_INVALID && op->mem.base != LYNX_RIP && op->mem.base != LYNX_RSP) {
                    addTaintToReg(backTaint, (LynxReg) op->mem.base, tid, curLabel);
                }

                if(op->mem.index != LYNX_INVALID && op->mem.index != LYNX_RIP && op->mem.index != LYNX_RSP) {
                    addTaintToReg(backTaint, (LynxReg) op->mem.index, tid, curLabel);
                }

                if(op->mem.seg != LYNX_INVALID) {
                    addTaintToReg(backTaint, (LynxReg) op->mem.seg, tid, curLabel);
                }
            }
            else {
                while(!dispatches.empty() && curDispatch.first == dispatches.back().first) {
                    dispatches.pop_back();
                }
            }
        }

        //for now check to see which is the first one executed, that is the one we will put the return on
        //might not work if we have an actual fork command because the forked thing might excute second
        if(taint != 0 && notMergedWithVip != 0 && labelSize(backTaint, taint) > 1) {
            for(uint8_t size : sizes) {
                uint64_t allActive = activeBySize[size];
                uint64_t sizeTaint = getLabelIntersect(backTaint, taint, sizeToDispatch[size]);

                uint64_t candidateFrom = getLabelIntersect(backTaint, sizeTaint, notMergedWithVip);
                notMergedWithVip = subtractLabel(backTaint, candidateFrom, notMergedWithVip);

                if(allActive != 0 && candidateFrom != 0) {
                    uint64_t from = popLabel(backTaint, &candidateFrom);
                    sizeTaint = subtractLabel(backTaint, from, sizeTaint);
                    uint64_t curActive = getLabelIntersect(backTaint, allActive, sizeTaint);
                    if(curActive != 0) {
                        activeBySize[size] = subtractLabel(backTaint, curActive, allActive);
                        printf(" < connecting %llx(%llx) to ", (unsigned long long) from, (unsigned long long) vip[from]);
                        uint64_t fromRet = vipRet[from];
                        while(curActive != 0) {
                            uint64_t to = popLabel(backTaint, &curActive);
                            uint64_t toRet = vipRet[to];

                            //check if returns are different
                            if(fromRet != from) {
                                controlFlow[make_pair(vip[fromRet], vip[from])]++;
                            }

                            //eliminate return self loops that yield statements
                            if(vip[from] == vip[to] && fromRet != from && toRet != to && fromRet != toRet) {
                                continue;
                            }

                            printf("%llx(%llx) ", (unsigned long long) to, (unsigned long long) vip[to]);
                            controlFlow[make_pair(vip[from], vip[to])]++;
                            fromRet = toRet;
                        }
                        vipRet[from] = fromRet;
                        printf("at %llu\n", (unsigned long long) curIns);
                        //curVips[from] = sizeTaint;
                    }
                }
            }
        }

        /*if(allActive != 0 && taint != 0) {
            uint64_t from = popLabel(backTaint, &taint);
            taint = getLabelIntersect(backTaint, active, taint);
            if(labelSize(backTaint, taint) > 0) {
                active = subtractLabel(backTaint, taint, active);
                printf(" < connecting %llx(%llx) to ", (unsigned long long) from, (unsigned long long) vip[from]);
                while(taint != 0) {
                    uint64_t to = popLabel(backTaint, &taint);
                    printf("%llx(%llx) ", (unsigned long long) to, (unsigned long long) vip[to]);
                    controlFlow[make_pair(vip[from], vip[to])]++;
                }
                printf("\n");
            }
        }*/
        
    }

    if(coalesce) {
        unordered_map<uint64_t, uint64_t> forwardEdges;
        unordered_map<uint64_t, uint64_t> backEdges;

        for(const pair< pair<uint64_t, uint64_t>, uint32_t> &edge : controlFlow) {
            forwardEdges[edge.first.first] = mergeLabels(forwardTaint, edge.first.second, forwardEdges[edge.first.first]);
            backEdges[edge.first.second] = mergeLabels(forwardTaint, edge.first.first, backEdges[edge.first.second]);
        }

        auto it = forwardEdges.begin();
        while(it != forwardEdges.end()) {
            if(labelSize(forwardTaint, it->second) == 1 && labelSize(forwardTaint, backEdges[it->second]) == 1) {
                uint64_t update = backEdges[it->first];
                uint64_t newBack = update;
                while(update != 0) {
                    uint64_t curNode = popLabel(forwardTaint, &update);
                    forwardEdges[curNode] = subtractLabel(forwardTaint, it->first, forwardEdges[curNode]);
                    forwardEdges[curNode] = mergeLabels(forwardTaint, it->second, forwardEdges[curNode]);
                    pair<uint64_t, uint64_t> oldEdge = make_pair(curNode, it->first);
                    uint32_t cnt = controlFlow[oldEdge];
                    controlFlow.erase(oldEdge);
                    controlFlow[make_pair(curNode, it->second)] = cnt;
                }

                backEdges[it->second] = newBack;
                backEdges.erase(it->first);
                uint32_t cnt = controlFlow[*it];
                controlFlow.erase(*it);
                it = forwardEdges.erase(it);
            }
            else {
                it++;
            }
        }
    }

    if(cfg == NULL) {
        fprintf(stderr, "Could not open cfg file\n");
    }
    else {
        fprintf(cfg, "digraph \"G\" {\n");
        set<uint64_t> blocks;
        for(auto it = controlFlow.begin(); it != controlFlow.end(); it++) {
            if(blocks.find(it->first.first) == blocks.end()) {
                blocks.insert(it->first.first);
                outputDot(cfg, it->first.first, cfgs[it->first.first], readerState);
                //fprintf(cfg, " B%llu [shape=box, style=solid, label=\"%llx\"];\n", (unsigned long long) it->first.first, (unsigned long long) it->first.first);
            }

            if(blocks.find(it->first.second) == blocks.end()) {
                blocks.insert(it->first.second);
                outputDot(cfg, it->first.second, cfgs[it->first.second], readerState);
                //fprintf(cfg, " B%llu [shape=box, style=solid, label=\"%llx\"];\n", (unsigned long long) it->first.second, (unsigned long long) it->first.second);
            }

            uint32_t from = getCFG(cfgs[it->first.first])->startingpoint->exit->id;
            uint32_t to = getCFG(cfgs[it->first.second])->startingpoint->entry->id;
            fprintf(cfg, " B%lld%d -> B%lld%d [style=solid, color=\"green\", label=\"%u\"];\n", (unsigned long long) it->first.first, from, (unsigned long long) it->first.second, to, it->second);
            //fprintf(cfg, " B%llu -> B%llu [style=solid, color=\"black\", label=\"%u\"];\n", (unsigned long long) it->first.first, (unsigned long long) it->first.second, it->second);
        }
        fprintf(cfg, "}\n");
    }


    freeTaint(forwardTaint);
    closeReader(readerState);

    return 0;
}
